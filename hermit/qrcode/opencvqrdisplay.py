...
class OpenCVQRDisplay(QRDisplay):
    ...
    def setup_camera_display(self):
        ...
        cv2.namedWindow(self.winname, cv2.WINDOW_NORMAL | cv2.WINDOW_GUI_NORMAL)

        # Move the window to a sane place on the screen
        cv2.moveWindow(self.winname, self.x_position, self.y_position)
    ...
    def display_camera_image(self, image):
        ...
        # RGB to BGR
        cvimg = cvimg[:, :, ::-1].copy()
        cv2.imshow(self.winname, cvimg)
        return window_is_open(self.winname, 100)
