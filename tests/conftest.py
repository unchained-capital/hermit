...
def fixture_opensource_shards(opensource_wallet_words):
    ...
    mock_shard1.to_str.return_value = "{0} ({1}/{2})".format(
        mock_shard1.name, mock_shard1.number, mock_shard1.count
    )
    mock_shard2.to_str.return_value = "{0} ({1}/{2})".format(
        mock_shard2.name, mock_shard2.number, mock_shard2.count
    )
    ...
    mock_shards = {mock_shard1.name: mock_shard1, mock_shard2.name: mock_shard2}
    return mock_shards
...
def prep_full_vector(filename):
    ...
    test_vector["expected_display"] = (
        test_vector["expected_display"]
        + json.dumps(test_vector["expected_signature"], indent=2)
        + "\n"
    )
    return test_vector
...
@pytest.fixture(
    params=[fixture_opensource_bitcoin_vector_0, fixture_opensource_bitcoin_vector_1]
)
